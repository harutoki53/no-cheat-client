-- Rivals Script: Final God Mode Hybrid (Everything Integrated)
...
mobileMenuBtn.Size = UDim2.new(0, 100, 0, 35); mobileMenuBtn.Position = UDim2.new(0, 10, 0, 50); mobileMenuBtn.Text = "MENU"; mobileMenuBtn.BackgroundColor3 = Color3.new(0, 0, 0); mobileMenuBtn.TextColor3 = Color3.new(1, 1, 0); Instance.new("UIStroke", mobileMenuBtn)
...
local function createESP(v)
    local container = Instance.new("Frame", gui); container.BackgroundTransparency = 1; container.Size = UDim2.new(1,0,1,0)
    local nameTag = Instance.new("TextLabel", container); nameTag.Size = UDim2.new(0, 150, 0, 20); nameTag.TextColor3 = Color3.new(1, 1, 1); nameTag.TextStrokeTransparency = 0; nameTag.BackgroundTransparency = 1; nameTag.Font = Enum.Font.RobotoMono; nameTag.TextScaled = true
    local hBarBG = Instance.new("Frame", container); hBarBG.Size = UDim2.new(0, 50, 0, 4); hBarBG.BackgroundColor3 = Color3.new(0, 0, 0)
    local hBar = Instance.new("Frame", hBarBG); hBar.Size = UDim2.new(1, 0, 1, 0); hBar.BackgroundColor3 = Color3.new(0, 1, 0); hBar.BorderSizePixel = 0
    return {Main = container, Name = nameTag, Bar = hBar, BarBG = hBarBG}
end
local pESP = {}
...
R.RenderStepped:Connect(function()
    ...
    for _, v in pairs(P:GetPlayers()) do
        if v ~= LP and v.Character and v.Character:FindFirstChild("Head") then
            local char = v.Character; local hum = char:FindFirstChildOfClass("Humanoid")
            if hum and hum.Health > 0 and (not v.Team or v.Team ~= LP.Team) then
                ...
                if onScreen then
                    ...
                    esp.Name.Position = UDim2.new(0, pos.X - 75, 0, pos.Y - 45); esp.Name.Text = string.format("%s [%dm]", v.DisplayName, math.floor(dist))
                    esp.BarBG.Position = UDim2.new(0, pos.X - 25, 0, pos.Y - 25); esp.Bar.Size = UDim2.new(hum.Health / hum.MaxHealth, 0, 1, 0)
