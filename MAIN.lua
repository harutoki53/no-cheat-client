-- Rivals Script: Harutoki Ultimate (Starting Point Fix)
...
-- --- GUI初期化 ---
local gui = LP:WaitForChild("PlayerGui"):FindFirstChild("HarutokiUltimate") or Instance.new("ScreenGui", LP.PlayerGui)
...
U.InputBegan:Connect(function(input, gpe)
    if input.KeyCode == Enum.KeyCode.RightShift and config.isPC then config.menuOpen = not config.menuOpen; updateUI() end
end)
...
R.RenderStepped:Connect(function()
    ...
    for _, v in pairs(P:GetPlayers()) do
        ...
        -- Headを優先し、なければRootPartをターゲットにする
        ...
        if head and hum and hum.Health > 0 then
            ...
            if dist <= config.maxDistance then
                ...
                if config.wallCheck then
                    ...
                    params.FilterDescendantsInstances = {LP.Character, char, C}
                ...
                if onScreen then
                    ...
                    -- エイム対象の選定 (FILTERがONならisVisibleが必要)
    ...
    if targetHead and config.aimbot then
        ...
        if isAim then
            ...
            -- オートファイア (念のためisVisibleを再確認するロジックを内包)
