...
local config = {
    aimbot = true,
    autoFire = true,
    wallCheck = true,    -- FILTER ONの時、壁越しは表示もエイムもしない
    isPC = true,
    smooth = 0.4,
    fov = 150,           -- スマホ用
    pcFov = 800,         -- PC用 (固定)
...
R.RenderStepped:Connect(function()
    ...
    for _, v in pairs(P:GetPlayers()) do
        if v ~= LP and v.Character and v.Character:FindFirstChild("Humanoid") then
            local char = v.Character; local hum = char.Humanoid; local root = char:FindFirstChild("HumanoidRootPart")
            if root and hum.Health > 0 and (not v.Team or v.Team ~= LP.Team) then
                local dist = (root.Position - LP.Character.PrimaryPart.Position).Magnitude
                ...
                if dist <= config.maxDistance then
                    ...
                    -- 【強化】フィルター (Raycastによる壁判定)
                    local isVisible = true
                    if config.wallCheck then
                        local params = RaycastParams.new()
                        params.FilterDescendantsInstances = {LP.Character, char}
                        params.FilterType = Enum.RaycastFilterType.Exclude
                        local result = workspace:Raycast(C.CFrame.Position, (char.Head.Position - C.CFrame.Position).Unit * dist, params)
